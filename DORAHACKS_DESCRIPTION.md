CasperFlow
A no-code visual builder for autonomous AI agents on Casper. You drag blocks onto a canvas, connect them, and get an agent that reads live on-chain data, decides with an AI model, and signs real Casper transactions on its own. No code, no terminal. And everything an agent can do on-chain is also published as an MCP server, so other AI tools can call the same Casper actions.
...
Every step is a real, final transaction you can open on the public Casper explorer. Nothing is faked.
...
TECH STACK
React 18, TypeScript and Vite, React Flow for the canvas, casper-js-sdk v5 for real TransactionV1 signing and submission, CSPR.cloud for on-chain reads, casper-eip-712 for attestations and x402, and a zero-dependency Node server for the x402 demo.
